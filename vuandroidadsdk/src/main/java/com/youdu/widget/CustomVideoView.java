...
public class CustomVideoView extends RelativeLayout implements View.OnClickListener,
    ...
    private static final int TIME_MSG = 0x01;  //时间类型
    private static final int TIME_INVAL = 1000;//时间间隔，每隔1000毫秒发一个TIME_MSG出来，也就是每隔一秒，
                                                // 执行下handler的handleMessage()这个方法
    //播放的状态
    private static final int STATE_ERROR = -1;
    private static final int STATE_IDLE = 0;
    private static final int STATE_PLAYING = 1;
    private static final int STATE_PAUSING = 2;
    //视频会加载失败，加载一次失败，视频播放的成功率比较低，加入了重试的机制
    ...
    private int mScreenWidth, mDestationHeight;//屏幕宽度

    /**
     * Status状态保护
     */
    private boolean canPlay = true;
    private boolean mIsRealPause;//是否暂停
    private boolean mIsComplete;//是否播放完成了
    private int mCurrentCount;
    private int playerState = STATE_IDLE;//默认处于空闲状态

    private MediaPlayer mediaPlayer;
    private ADVideoPlayerListener listener;//事件监听回调
    private ScreenEventReceiver mScreenReceiver;//接受屏幕是否锁屏

    /**
     * 每隔一秒 TIME_INVAL，发送一个TIME_MSG事件
     *
     */
    ...
    /**
     * 播放器处于就绪状态，只有播放器处于就绪，再start,不然没意义
     * @param mp
     */
    @Override
    ...
    /**
     * 只有TextureView处于Available状态时，才能加载帧数据
     * @param surface
     * @param width
     * @param height
     */
